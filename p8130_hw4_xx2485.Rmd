...
sign_test_result <- SIGN.test(data, md = 120, alternative = "less")
...
Since the p-value (\(0.2706\)) is greater than the significance level (\(\alpha = 0.05\)), we fail to reject the null hypothesis. 
...
The test statistic is 10 as we use the smaller number of + and - signs, given n (number of trials) <= 25.


Perform Wilcoxon signed-rank test.
```{r}
# Perform the Wilcoxon signed-rank test to check if the median blood sugar level is less than 120
# H0: median = 120
# HA: median < 120
# Set alternative = "less" for a one-sided test
wilcoxon_test_result = wilcox.test(data, mu = 120, alternative = "less")

wilcoxon_test_result
```
Since the p-value (\(0.1447\)) is greater than the significance level (\(\alpha = 0.05\)), we fail to reject the null hypothesis.
...
The p-value for the slope coefficient of `ERvisits` is `r signif(summary(log_model)$coefficients[2, 4], 4)`, which is less than the significance level of 0.05. This indicates that the slope is statistically significant. Therefore, there is strong evidence that the number of ER visits is associated with the log-transformed total cost.


The regression equation based on the output is:
\[
\text{Log(Total Cost + 1)} = `r signif(summary(log_model)$coefficients[1, 1], 4)` + `r signif(summary(log_model)$coefficients[2, 1], 4)` \cdot \text{ERvisits}
\]


The slope coefficient for `ERvisits` is `r signif(summary(log_model)$coefficients[2, 1], 4)`. This means that for every additional ER visit, the log-transformed total cost is expected to increase by approximately `r signif(summary(log_model)$coefficients[2, 1], 4)` units on average.
...
From the regression model, the interaction term `heart_data$ERvisits:heart_data$comp_bin` has a coefficient of `r signif(-0.09753, 4)` and a p-value of `r signif(0.311, 4)`. 
...
F-statistic and overall significance:
   ...
   - The overall F-statistic is `r signif(summary(mlr_model_extended)$fstatistic[1], 4)`, with a p-value of `r format.pval(summary(mlr_model_extended)$fstatistic[3], 4)` (\( p < 0.05 \)).
...
1. ERvisits:
   ...
   - Coefficient: `r signif(summary(mlr_model_extended)$coefficients["ERvisits", 1], 4)`
   
   - \( p \)-value: `r signif(summary(mlr_model_extended)$coefficients["ERvisits", 4], 4)`
   
   - Interpretation: ER visits are significantly increase log-transformed total cost (\( p < 0.05 \)).

2. comp_bin:

   - Coefficient: `r signif(summary(mlr_model_extended)$coefficients["comp_bin", 1], 4)`
   
   - \( p \)-value: `r signif(summary(mlr_model_extended)$coefficients["comp_bin", 4], 4)`
   
   - Interpretation: Complications significantly increase log-transformed total cost (\( p < 0.05 \)).

3. age:

   - Coefficient: `r signif(summary(mlr_model_extended)$coefficients["age", 1], 4)`
   
   - \( p \)-value: `r signif(summary(mlr_model_extended)$coefficients["age", 4], 4)`
   
   - Interpretation: Age significantly increase log-transformed total cost (\( p < 0.05 \)).

4. gender:

   - Coefficient: `r signif(summary(mlr_model_extended)$coefficients["genderFemale", 1], 4)`
   
   - \( p \)-value: `r signif(summary(mlr_model_extended)$coefficients["genderFemale", 4], 4)`
   
   - Interpretation: Gender is not statistically significant (\( p = 0.1364 \)).

5. duration:

   - Coefficient: `r signif(summary(mlr_model_extended)$coefficients["duration", 1], 4)`
   
   - \( p \)-value: `r signif(summary(mlr_model_extended)$coefficients["duration", 4], 4)`
   
   - Interpretation: Duration significantly increase log-transformed total cost (\( p < 0.05 \)).
   
The model explains approximately `r signif(summary(mlr_model_extended)$r.squared * 100, 2)`% of the variability in log-transformed total cost. `ERvisits`, `comp_bin`, `age`, and `duration` are statistically significant, while `gender` is not. Duration has the strongest positive association with log-transformed total cost, followed by `comp_bin` and `ERvisits`.
...
1. Residual Standard Error: `r signif(summary(log_model)$sigma, 4)`

2. \( R^2 \): `r signif(summary(log_model)$r.squared, 4)` (approximately `r signif(summary(log_model)$r.squared * 100, 2)`% of variability in total cost explained by ER visits alone).

3. ERvisits Coefficient:
   - Estimate: `r signif(summary(log_model)$coefficients["ERvisits", 1], 4)`
   - \( p \)-value: `r signif(summary(log_model)$coefficients["ERvisits", 4], 4)`
...
1. Residual Standard Error: `r signif(summary(mlr_model_extended)$sigma, 4)`

2. \( R^2 \): `r signif(summary(mlr_model_extended)$r.squared, 4)` (approximately `r signif(summary(mlr_model_extended)$r.squared * 100, 2)`% of variability in total cost explained by ER visits along with other predictors).

3. Adjusted \( R^2 \): `r signif(summary(mlr_model_extended)$adj.r.squared, 4)`.

4. ERvisits Coefficient:
   - Estimate: `r signif(summary(mlr_model_extended)$coefficients["ERvisits", 1], 4)`
   - \( p \)-value: `r signif(summary(mlr_model_extended)$coefficients["ERvisits", 4], 4)`
...
The **MLR model** would be used to address the objective because it has a larger \( R^2 \) value. \( R^2 \) represents the proportion of variance explained by model, and there is a significant improvement in explanatory power when additional covariates are included.
