...
left-skewed, but after log transformation, it follows roughly a normal
distribution shape.
...
total cost and ER visits, the **SLR model** would be used to address the
objective. It directly quantifies the relationship without adjusting for
additional variables, making it simpler and more interpretable for this
specific research question. However, if confounders like `comp_bin` are
taken into consideration, the **MLR model** would be more appropriate.
